...
const mapProps = (state: IStore) => ({
  contactScreenState: state.contactScreen.state,
});
...
export function Application(props: IApplicationProps) {
  ...
  return (
    <React.Fragment>
      <Menu />
      <Parallax
        pages={5 + footerPercent}
        ref={(ref: IExtParallax) => setScrollHandler(ref, dispatch)}
        scrolling
      >
        ...
        <div className="fon-section" onScroll={(e) => console.warn(e)}>
          ...
          <ScreenCommutator
            ...
            screens={[Join, ContactUsScreen, Customer]}
          />
          <Footer />
        </div>
      </Parallax>
